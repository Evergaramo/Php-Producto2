<?php
	...
	$stmt = $conn->prepare("SELECT A.name, A.color, B.time_start, B.time_end, B.day 
	...
	while ($j=($result->fetch_assoc())) {
		$myJson =  $myJson . json_encode($j) . ','; // Parse to JSON and print.
	}
...
var data = [],
  ...
  //LECTURA JSON DE LA CONSULTA
  //Ejemplo json -> {'clases':[{"name":"mates clase","color":"rojo","time_start":"11:25:00","time_end":"13:00:00","day":"2019-06-06"}]}
  var json_datos = <?php echo $myJson;?>;
  var clases = json_datos['clases'];
  for (var i = 0; i < clases.length; i++) {
	  var name = clases[i].name;
	  var color = clases[i].color;
	  //--//
	  var time_start = clases[i].time_start;
	  var separador = ":";
	  var arregloDeSubCadenas = time_start.split(separador);
	  var h_start = arregloDeSubCadenas[0];
	  var m_start = arregloDeSubCadenas[1];
	  //--//
	  var time_end = clases[i].time_end;
	  separador = ":";
	  arregloDeSubCadenas = time_end.split(separador);
	  var h_end = arregloDeSubCadenas[0];
	  var m_end = arregloDeSubCadenas[1];
	  //--//
	  var day = clases[i].day;
	  separador = "-";
	  arregloDeSubCadenas = day.split(separador);
	  var ano = arregloDeSubCadenas[0];
	  var mes = arregloDeSubCadenas[1];
	  var dia = arregloDeSubCadenas[2];
	  //--//
	  data.push({ title: name, start: new Date(ano, mes-1, dia, h_start, m_start), end: new Date(ano, mes-1, dia, h_end, m_end), allDay: 0, text: color });
  }
